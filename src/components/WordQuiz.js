    ...
    return(
        <div style={{height:'100%'}}>
            ...
            <div style={{display:'flex', flexDirection:'column', justifyContent:'flex-start', alignItems:'flex-start', height:'40%'}}>
                <div style={{marginTop: '30px', padding:'0.5rem 8rem', fontSize:'20px', color: 'white'}} onMouseOver={(e) => e.target.style.color = 'limegreen'} onMouseOut={(e) => e.target.style.color = 'white'}>SELECT * FROM 긴글 코딩 연습;</div>
                ...
                <div style={{padding:'0.5rem 8rem', fontSize:'20px', color: 'white'}} onMouseOver={(e) => e.target.style.color = 'limegreen'} onMouseOut={(e) => e.target.style.color = 'white'}>SELECT * FROM 개념 퀴즈;</div>
            </div>
